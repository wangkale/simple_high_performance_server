...
CSocekt::CSocekt()
{
    m_worker_connections = 1;                                //最大连接数
    m_ListenPortCount = 1;                                        //监听端口
    m_RecyConnectionWaitTime = 60;                   //等待回收连接的时长
    m_epollhandle = -1;                                                             //epoll返回的句柄
    m_iLenPkgHeader = sizeof(COMM_PKG_HEADER);    //包头的大小
    m_iLenMsgHeader =  sizeof(STRUC_MSG_HEADER);  //消息头的大小
    m_iSendMsgQueueCount     = 0;                           //发送消息队列的大小
