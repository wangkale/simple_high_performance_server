...
{		
	...
	struct sockaddr           s_sockaddr;                             //客户端的地址
	...
	//和收包有关
	unsigned char             curStat;                                             //收包的状态
	char                      dataHeadInfo[_DATA_BUFSIZE_];     //包头数据存放			
	char                      *precvbuf;                                               //接收数据的缓冲区的头指针，对收到不全的包非常有用，看具体应用的代码
	unsigned int              irecvlen;                                            //要收数据大小
...
{
	lpngx_connection_t pConn;      //保存连接
	uint64_t           iCurrsequence;   //连接的序号
